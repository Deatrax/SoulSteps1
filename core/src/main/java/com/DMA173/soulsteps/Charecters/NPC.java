...
public class NPC extends Character {
    ...
    public void interact(Player player, GameStateManager gsm) {
        ...
        if (this.getName().equals("Lena") && !gsm.hasCompletedObjective("talked_to_lena_first_time")) {
            System.out.println(name + ": " + dialogue);
            player.adjustKindness(5); // Being an ally gives kindness
            gsm.completeObjective("talked_to_lena_first_time"); // IMPORTANT: This triggers the story progression!
            this.setDialogue("Did you find anything yet?"); // Change dialogue for next time
        } else {
